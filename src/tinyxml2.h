...
#define TIXMLASSERT(x)                                                \
	do {                                                          \
		if (!(x)) {                                           \
			__android_log_assert("assert", "grinliz",     \
					     "ASSERT in '%s' at %d.", \
					     __FILE__, __LINE__);     \
		}                                                     \
	} while (false)
...
public:
	enum Mode {
		...
		TEXT_ELEMENT = NEEDS_ENTITY_PROCESSING |
			       NEEDS_NEWLINE_NORMALIZATION,
		TEXT_ELEMENT_LEAVE_ENTITIES = NEEDS_NEWLINE_NORMALIZATION,
		ATTRIBUTE_NAME = 0,
		ATTRIBUTE_VALUE = NEEDS_ENTITY_PROCESSING |
				  NEEDS_NEWLINE_NORMALIZATION,
...
public:
	...
	void Trace(const char *name)
	{
		printf("Mempool %s watermark=%d [%dk] current=%d size=%d nAlloc=%d blocks=%d\n",
		       name, _maxAllocs, _maxAllocs * ITEM_SIZE / 1024,
		       _currentAllocs, ITEM_SIZE, _nAllocs, _blockPtrs.Size());
	}
...
public:
	...
	{
		return const_cast<char *>(SkipWhiteSpace(
			const_cast<const char *>(p), curLineNumPtr));
	}
...
public:
    ...
    */
	...
	XMLElement *FirstChildElement(const char *name = 0)
	{
		return const_cast<XMLElement *>(
			const_cast<const XMLNode *>(this)->FirstChildElement(
				name));
	}
    ...
    */
	...
	{
		return const_cast<XMLElement *>(
			const_cast<const XMLNode *>(this)->LastChildElement(
				name));
	}
	...
	{
		return const_cast<XMLElement *>(
			const_cast<const XMLNode *>(this)->NextSiblingElement(
				name));
	}
...
protected:
	...
	virtual char *ParseDeep(char *p, StrPair *parentEndTag,
				int *curLineNumPtr);
...
protected:
	...
	char *ParseDeep(char *p, StrPair *parentEndTag,
			int *curLineNumPtr) override;
...
public:
    ...
    */
	...
	/// See QueryIntAttribute()
	XMLError QueryUnsigned64Attribute(const char *name,
...
public:
	...
	{
		return XMLHandle(_node ? _node->PreviousSiblingElement(name)
				       : 0);
	}
...
public:
	...
	{
		return XMLConstHandle(_node ? _node->FirstChildElement(name)
					    : 0);
	}
	...
	{
		return XMLConstHandle(_node ? _node->LastChildElement(name)
					    : 0);
	}
	...
	{
		return XMLConstHandle(
			_node ? _node->PreviousSiblingElement(name) : 0);
	}
	...
	{
		return XMLConstHandle(_node ? _node->NextSiblingElement(name)
					    : 0);
	}
