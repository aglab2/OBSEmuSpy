	...
	if (_flags & NEEDS_FLUSH) {
		*_end = 0;
		...
		if (_flags) {
			...
			while (p < _end) {
				if ((_flags & NEEDS_NEWLINE_NORMALIZATION) &&
				    *p == CR) {
					...
					*q = LF;
					++q;
				} else if ((_flags & NEEDS_ENTITY_PROCESSING) &&
					   *p == '&') {
					...
					if (*(p + 1) == '#') {
						...
						char buf[buflen] = {0};
						int len = 0;
						const char *adjusted = const_cast<
							char *>(
...
{
	TIXML_SNPRINTF(buffer, bufferSize, "%s",
...
char *XMLDocument::Identify(char *p, XMLNode **node, bool first)
{
	...
	if (XMLUtil::StringEqual(p, xmlHeader, xmlHeaderLen)) {
		...
		if (WhitespaceMode() == PEDANTIC_WHITESPACE && first &&
		    p != start && *(p + elementHeaderLen) == '/') {
			...
			returnNode =
				CreateUnlinkedNode<XMLElement>(_elementPool);
			returnNode->_parseLineNum = _parseCurLineNum;
			p += elementHeaderLen;
		}
	} else {
...
char *XMLNode::ParseDeep(char *p, StrPair *parentEndTag, int *curLineNumPtr)
{
	...
	while (p && *p) {
		...
		if (!p) {
			...
			if (!_document->Error()) {
				_document->SetError(XML_ERROR_PARSING,
						    initialLineNum, 0);
			}
			break;
		}
		...
		if (ele) {
			...
			if (endTag.Empty()) {
				...
				if (ele->ClosingType() != XMLElement::OPEN) {
					mismatch = true;
				} else if (!XMLUtil::StringEqual(endTag.GetStr(),
								 ele->Name())) {
					mismatch = true;
				}
			}
			if (mismatch) {
				_document->SetError(
					...
					initialLineNum, "XMLElement name=%s",
					ele->Name());
				_document->DeleteNode(node);
				break;
			}
...
char *XMLText::ParseDeep(char *p, StrPair *, int *curLineNumPtr)
{
	if (this->CData()) {
		p = _value.ParseText(p, "]]>",
				     ...
				     curLineNumPtr);
		if (!p) {
			_document->SetError(XML_ERROR_PARSING_CDATA,
					    _parseLineNum, 0);
		}
		return p;
	} else {
		...
		if (!p) {
			_document->SetError(XML_ERROR_PARSING_TEXT,
					    _parseLineNum, 0);
		}
	}
	return 0;
...
char *XMLComment::ParseDeep(char *p, StrPair *, int *curLineNumPtr)
{
	...
	if (p == 0) {
		_document->SetError(XML_ERROR_PARSING_COMMENT, _parseLineNum,
				    0);
	}
	return p;
}
...
bool XMLDeclaration::ShallowEqual(const XMLNode *compare) const
{
	...
	return (declaration &&
		XMLUtil::StringEqual(declaration->Value(), Value()));
}
...
char *XMLUnknown::ParseDeep(char *p, StrPair *, int *curLineNumPtr)
{
	...
	p = _value.ParseText(p, ">", StrPair::NEEDS_NEWLINE_NORMALIZATION,
			     curLineNumPtr);
	if (!p) {
		_document->SetError(XML_ERROR_PARSING_UNKNOWN, _parseLineNum,
				    0);
	}
	return p;
}
...
char *XMLElement::ParseAttributes(char *p, int *curLineNumPtr)
{
	...
	while (p) {
		p = XMLUtil::SkipWhiteSpace(p, curLineNumPtr);
		if (!(*p)) {
			_document->SetError(XML_ERROR_PARSING_ELEMENT,
					    _parseLineNum, "XMLElement name=%s",
					    Name());
			return 0;
		}
		...
		if (XMLUtil::IsNameStartChar(static_cast<unsigned char>(*p))) {
			...
			if (!p || Attribute(attrib->Name())) {
				...
				_document->SetError(XML_ERROR_PARSING_ATTRIBUTE,
						    attrLineNum,
						    "XMLElement name=%s",
						    Name());
				return 0;
...
void XMLDocument::Clear()
{
	...
	while (_unlinked.Size()) {
		DeleteNode(
			_unlinked[0]); // Will remove from _unlinked as part of delete.
	}
...
#ifdef TINYXML2_DEBUG
	if (!hadError) {
		TIXMLASSERT(_elementPool.CurrentAllocs() ==
			    _elementPool.Untracked());
		TIXMLASSERT(_attributePool.CurrentAllocs() ==
			    _attributePool.Untracked());
		TIXMLASSERT(_textPool.CurrentAllocs() == _textPool.Untracked());
		TIXMLASSERT(_commentPool.CurrentAllocs() ==
...
XMLError XMLDocument::LoadFile(const char *filename)
{
	if (!filename) {
		TIXMLASSERT(false);
		SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0,
			 "filename=<null>");
		return _errorID;
	}
...
XMLError XMLDocument::SaveFile(const char *filename, bool compact)
{
	...
	if (!fp) {
		SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0, "filename=%s",
			 filename);
		return _errorID;
	}
	SaveFile(fp, compact);
...
XMLPrinter::XMLPrinter(FILE *file, bool compact, int depth,
		       EscapeAposCharsInAttributes aposInAttributes)
	  ...
	  _buffer()
{
	...
	for (int i = 0; i < NUM_ENTITIES; ++i) {
		...
		if ((aposInAttributes == ESCAPE_APOS_CHARS_IN_ATTRIBUTES) ||
		    (entityValue != SINGLE_QUOTE)) {
			const unsigned char flagIndex =
...
void XMLPrinter::Print(const char *format, ...)
{
	...
	if (_fp) {
		...
		TIXMLASSERT(len >= 0);
		va_start(va, format);
		TIXMLASSERT(_buffer.Size() > 0 &&
			    _buffer[_buffer.Size() - 1] == 0);
		char *p = _buffer.PushArr(len) -
...
void XMLPrinter::Putc(char ch)
{
	if (_fp) {
		...
		char *p = _buffer.PushArr(sizeof(char)) -
			  1; // back up over the null terminator.
		p[0] = ch;
		p[1] = 0;
	}
...
void XMLPrinter::PrintString(const char *p, bool restricted)
{
	...
	if (_processEntities) {
		const bool *flag = restricted ? _restrictedEntityFlag
					      : _entityFlag;
		while (*q) {
			TIXMLASSERT(p <= q);
		...
		if (p < q) {
			const size_t delta = q - p;
			const int toPrint = (INT_MAX < delta)
						    ? INT_MAX
			    ...
			    const XMLAttribute *attribute)
{
	...
	const bool compactMode = parentElem ? CompactMode(*parentElem)
					    : _compactMode;
